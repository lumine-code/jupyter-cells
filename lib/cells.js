...
function getRegexString(editor) {
  ...
  const percentMarker = `${escapedCommentStartString}[\t ]*%{2,}(?:[\t ]+(\\[(?:md|markdown)\\]|md|markdown)(?=[\t \r\n]|$))?`;
  const regexString = `${percentMarker}| *<(codecell|md|markdown)>| *(In[[0-9 ]*])`;
  return regexString;
}
...
function getMarkerIndex(editor) {
  ...
  buffer.scan(new RegExp(regexString, "g"), ({ match, range }) => {
    ...
    for (let i = 1; i < match.length; i++) {
      if (!match[i]) continue;
      const metadata = match[i].replace(/^\[|\]$/g, "");
      type = metadata === "md" || metadata === "markdown" ? "markdown" : "codecell";
    }
